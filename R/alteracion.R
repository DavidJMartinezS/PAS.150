...
#'   para que un fragmento seccionado sea considerado "alterado". Por defecto es 0.5 ha.
#'
...
get_alter_bnp_remanente <- function(BNP, obras, sp, sup_ha_max = 0.5) {
  ...
  valid_input(BNP, inherit = "sf", names = c("Nom_ssubc", "BNP_ECC"))
  valid_input(obras, inherit = "sf")
  ...
  alteracion <- remanentes %>% 
    ...
    dplyr::arrange(Sup_ha) %>% 
    dplyr::select(dplyr::all_of(names(sf::st_drop_geometry(BNP))), Afectacion, Sup_ha) %>%
    dplyr::relocate(geometry, .after = dplyr::last_col()) %>% 
    suppressMessages() %>% 
    suppressWarnings()
...
# censos <- read_sf("~/CNX955/00. Insumos/03. Compilado censo/Compilado_20260527/Compilado_censos_20260527.shp")
get_alter_censo_buffer <- function(BNP, obras, censos, sp, buffer = 5, dTolerance = 0, radio = 1){
  BNP <- tryCatch(
    PAS.150:::prepare_uso_veg(x = BNP),
    error = function(e) stop("Error al preparar la capa de BNP", e$message, call. = FALSE)
  )
  ...
  alter <- ind %>% 
    ...
    sf::st_intersection(
      BNP %>% 
        PAS.150:::prepare_uso_veg() %>% 
        filter(BNP_ECC %>% stringi::stri_detect_regex(sp, case_insensitive = T)) %>% 
        rename(ECC1_Art19 = ECC1_ART19 , ECC2_Art19 = ECC2_ART19, F_Ley20283 = F_ley20283) %>% 
        select(-Obra)
    ) %>%
    sf::st_collection_extract("POLYGON") %>% 
    ...
    sf::st_collection_extract("POLYGON") %>% 
    sf::st_cast("POLYGON") %>%
    sf::st_join(obras %>% dplyr::select(Obra, Nom_obra, Obra_detal, Tipo), join = sf::st_nearest_feature) %>% 
    sf::st_intersection(ssubc_T2 %>% dplyr::select(ID_SSUBC, NOM_SSUBC)) %>%  
    sf::st_collection_extract("POLYGON") %>% sf::st_make_valid() %>% 
    sf::st_collection_extract("POLYGON") %>% sf::st_cast("POLYGON") %>% 
    dplyr::mutate(
      Afectacion = "Alteracion 2",
      Sup_ha = sf::st_area(geometry) %>% units::set_units(ha) %>% units::drop_units() %>% janitor::round_half_up(2),
      Sup_m2 = sf::st_area(geometry) %>% units::drop_units() %>% janitor::round_half_up() %>% as.integer()
    ) %>% 
    select(names_afect) %>% 
