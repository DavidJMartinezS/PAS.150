...
BD_fragmentacion <- function(
  ...
  stopifnot(
    "Extensión de archivos no válida. 'xls' o 'xlsx'." = tools::file_ext(
      path_frag
    ) %in%
      c("xls", "xlsx")
  )
  ...
  if (!is.null(spp_acomp)) {
    valid_df(df = spp_acomp)
  }
  if (!is.null(prop)) {
    valid_df(df = prop, names = c("Proporciones", "Estado"))
  }
  if (!is.null(estadisticos)) {
    valid_df(df = estadisticos, names = c("Variable"))
  }
  valid_input(portada_opts, inherit = "list")

  # FragStats ----
  resultados_antes <- {
    if (tools::file_ext(path_frag) == "xls") {
      readxl::read_xls(path_frag, sheet = 1)
    } else {
      openxlsx2::read_xlsx(path_frag, sheet = 1)
    }
  } %>%
  ...
  resultados_despues <- {
    if (tools::file_ext(path_frag) == "xls") {
      ...
      openxlsx2::read_xlsx(path_frag, sheet = 2)
    }
  } %>%
    dplyr::mutate(CA = AREA, NP = 1, PID = as.character(PID)) %>%
    dplyr::select(PID, NP, CA, AREA, SHAPE, FRAC, CORE, NCORE, PROX, ENN)
      ...
      dplyr::bind_rows(
        x %>%
          ...
          dplyr::bind_cols(
            x %>%
              dplyr::summarise_at(
                dplyr::vars(AREA, SHAPE, FRAC, PROX, ENN),
                .funs = mean
              ) %>%
  ...
  eval_frag <- function(SIGLA, DIFF, CLASE) {
    ...
    if (SIGLA == 'NP') {
      val <- ifelse(
        CLASE == "Aumento",
        ifelse(DIFF > 10, 1, ifelse(DIFF > 5, 2, ifelse(DIFF > 1, 4, 8))),
        8
      )
    }
    if (SIGLA %in% c('CA', 'PROX')) {
      val <- ifelse(
        CLASE == "Reducción",
        ifelse(DIFF > 5, 1, ifelse(DIFF > 1, 2, ifelse(DIFF < 1, 3, 4))),
        4
      )
    }
    if (SIGLA == 'AREA') {
      val <- ifelse(
        CLASE == "Reducción",
        ifelse(DIFF > 5, 1, ifelse(DIFF > 1, 2, ifelse(DIFF < 1, 2, 4))),
        4
      )
    }
    if (SIGLA %in% c('SHAPE', 'FRAC', 'NCORE', 'ENN')) {
      val <- ifelse(
        CLASE == "Aumento",
        ifelse(DIFF > 5, 1, ifelse(DIFF > 1, 2, ifelse(DIFF < 1, 3, 4))),
        4
    ...
    if (SIGLA == 'CORE') {
      val <- ifelse(
        CLASE == "Reducción",
        ifelse(DIFF > 5, 1, ifelse(DIFF > 1, 2, ifelse(DIFF < 1, 3, 6))),
        6
      )
    }
    return(val)
  ...
  df <- data.frame(
    ...
    dplyr::mutate(
      ...
      CLASE = ifelse(
        ANTES == `DESPUÉS`,
        "Igual",
        ifelse(ANTES < `DESPUÉS`, "Aumento", "Reducción")
      )
    ) %>%
    dplyr::mutate_at(2:3, janitor::round_half_up, 3) %>%
    mutate_at(4, janitor::round_half_up, 2) %>%
    dplyr::mutate(
  ...
  matriz_paisaje <- Usos %>%
    ...
    dplyr::full_join(
      Usos %>%
        ...
        dplyr::select(-n) %>%
        dplyr::mutate(
          `Intervención` = sf::st_area(geometry) %>%
            units::set_units(ha) %>%
            units::drop_units() %>%
    ...
    dplyr::mutate(
      Tasa = (log(`Después` / Antes) * 100) %>% janitor::round_half_up(2)
    ) # añadir tasa

  densidades_prop <- if (is.null(prop)) {
    NULL
  } else {
    prop %>%
      dplyr::mutate(
        Promedio = (estadisticos[estadisticos$Variable == ECC, ][["Promedio"]] *
          Proporciones) %>%
  ...
  eval_param <- function(param, valor) {
    ...
    if (param == 'Matriz del paisaje (ha)') {
      val <- ifelse(
        abs(matriz_paisaje[
          matriz_paisaje$Subuso == 'Total vegetación',
          'Tasa'
        ]) >
          5,
        1,
    ...
    if (param == 'Hábitat natural') {
      val <- dplyr::case_match(RCE, "CR" ~ 1, "EN" ~ 5, "VU" ~ 8)
    }
    if (param == 'Riqueza de especies') {
      val <- if (is.null(spp_acomp)) {
        1
      } else {
        dplyr::case_match(nrow(spp_acomp), 0 ~ 3, 1 ~ 2, .default = 1)
      }
    }
    if (param == 'Abundancia de especies') {
      val <- if (is.null(spp_acomp)) {
        1
      } else {
        ifelse(nrow(spp_acomp) >= 1, 1, 3)
      }
    }
    if (param == 'Regeneración') {
      if (is.null(densidades_prop)) {
        ...
        densidades_prop$Estado %>%
          stringi::stri_detect_regex("regenera", case_insensitive = TRUE) %>%
          any()
      ) {
        val <- ifelse(
          densidades_prop[
    ...
    if (param == 'Brinzales') {
      if (is.null(densidades_prop)) {
        ...
        densidades_prop$Estado %>%
          stringi::stri_detect_regex("brinzal", case_insensitive = TRUE) %>%
          any()
      ) {
        val <- ifelse(
          densidades_prop[
    ...
    if (param == 'Árboles adultos') {
      val <- if (is.null(densidades_prop)) {
        1
      } else {
        ifelse(
          densidades_prop[densidades_prop$Estado == "Adulto", "Int_inf"] > 150,
          9,
          ifelse(
            densidades_prop[densidades_prop$Estado == "Adulto", "Int_inf"] > 50,
            5,
            1
          )
        )
    ...
    if (param == 'Alteración') {
      val <- ifelse(alteracion, 1, 2)
    }
    if (param == 'Extensión de la presencia') {
      val <- 3
    }
    if (param == 'Sistema reproductivo') {
      val <- dplyr::case_match(Tipo_rep, 'dioica' ~ 2, "monoica" ~ 4)
    }
    return(val)
  }
  ...
  tabla_eval <- tabla_guia_fragmentacion %>%
    ...
    dplyr::select(-n) %>%
    dplyr::rename_all(stringi::stri_trans_toupper) %>%
    merge(df[, c(2, 7:8)], by = 'PARÁMETRO', all = TRUE) %>%
    dplyr::mutate(
      `PONDERACIÓN` = dplyr::case_match(
        ESCALA_ESPACIAL,
        'PAISAJE' ~ 0.2,
        'HÁBITAT' ~ 0.5,
        .default = 0.3
    ...
    tibble::as_tibble() %>%
    dplyr::mutate(
      `VALORACIÓN` = purrr::map2_dbl(`PARÁMETRO`, `VALORACIÓN`, eval_param)
    ) %>%
    dplyr::inner_join(
      tabla_guia_fragmentacion[, c(3, 5, 6)] %>%
        dplyr::rename_all(stringi::stri_trans_toupper),
      by = c('PARÁMETRO', 'VALORACIÓN')
    ) %>%
    dplyr::rename_all(stringi::stri_trans_totitle, type = "sentence") %>%
    dplyr::rename(
      `Categoría observada` = `Categorías`,
      Valores = `Valoración`
    ) %>%
...
VALORACIÓN; CATEGORÍAS: Valores con sus respectivas categorías de acuerdo con la indicado en la Guía antes mencionada"
  ...
  wb <- wb %>%
    openxlsx2::wb_add_data(
      x = nota,
      start_col = 1,
      start_row = nrow(desc_general) + 2 + nrow(desc_h1),
      col_names = FALSE
    ) %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(
        cols = 1:2,
        rows = nrow(desc_general) + 2 + nrow(desc_h1)
      ),
      italic = TRUE,
      size = 9
    ) %>%
...
Tasa: Porcentaje de pérdida de superficie. Calculado como log(Después/Antes)*100."
  ...
  wb <- wb %>%
    openxlsx2::wb_add_data(
      x = desc_h2,
      start_col = 1,
      start_row = nrow(desc_general) + 2 + nrow(desc_h1) + 2,
      col_names = FALSE
    )
  desc_h3 <- c(
  ...
  wb <- wb %>%
    ...
    openxlsx2::wb_merge_cells(
      dims = openxlsx2::wb_dims(cols = 1:ncol(resultados_antes), rows = 1),
      solve = TRUE
    ) %>%
    # Datos antes
    openxlsx2::wb_add_data(
    ...
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(resultados_antes)),
      bold = TRUE
    ) %>%
    openxlsx2::wb_add_fill(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(resultados_antes)),
    ...
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(resultados_antes),
        rows = 1:(nrow(resultados_antes) + 3)
      ),
      inner_hgrid = "thin",
      inner_vgrid = "thin",
    ...
    openxlsx2::wb_add_data(
      x = "Total antes",
      start_col = 1,
      start_row = nrow(resultados_antes) + 3
    )
  for (i in c(2:10)) {
    j <- LETTERS[i]
  ...
  wb <- wb %>%
    ...
    openxlsx2::wb_merge_cells(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(resultados_despues) + 11,
        rows = 1
      ),
      solve = TRUE
    ) %>%
    ...
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(
        rows = 2,
        cols = 1:ncol(resultados_despues),
        from_col = "L"
      ),
      bold = TRUE
    ) %>%
    ...
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(resultados_despues) + 11,
        rows = 1:(nrow(resultados_despues) + 3)
      ),
      inner_hgrid = "thin",
      inner_vgrid = "thin",
      inner_hcolor = openxlsx2::wb_color(hex = "#6F7B62"),
    ...
    openxlsx2::wb_add_data(
      x = "Total después",
      start_col = 12,
      start_row = nrow(resultados_despues) + 3
    )
  for (i in c(2:10)) {
    j <- LETTERS[i + 11]
    fx <- ifelse(i %in% c(2, 3, 7, 8), "SUM", "AVERAGE")
  ...
  wb <- wb %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(resultados_despues) + 11,
        rows = nrow(resultados_despues) + 3
      ),
      bold = TRUE
    ) %>%
    # Parametros
    openxlsx2::wb_add_data(
      x = df %>%
        dplyr::mutate_at(3:6, ~ ifelse(is.numeric(.), NA, NA_character_)),
      dims = openxlsx2::wb_dims(rows = nrow(resultados_antes) + 5, cols = 1)
    ) %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(
        cols = seq_len(ncol(df)),
        rows = nrow(resultados_antes) + 5
      ),
      bold = TRUE
    ) %>%
    openxlsx2::wb_add_fill(
    ...
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(df),
        rows = 1:(nrow(df) + 1),
        from_row = nrow(resultados_antes) + 5
      ),
      inner_hgrid = "thin",
      inner_vgrid = "thin",
      inner_hcolor = openxlsx2::wb_color(hex = "#6F7B62"),
  ...
  wb <- wb %>%
    ...
    openxlsx2::wb_add_data(
      x = "Superficies (ha) x Subuso de suelo",
      dims = "A1"
    ) %>%
    openxlsx2::wb_add_font(
      dims = "A1",
    ...
    openxlsx2::wb_add_cell_style(dims = "A1", horizontal = "center") %>%
    openxlsx2::wb_merge_cells(
      dims = openxlsx2::wb_dims(cols = 1:ncol(matriz_paisaje), rows = 1),
      solve = TRUE
    ) %>%
    openxlsx2::wb_add_data(
      x = matriz_paisaje %>%
        dplyr::mutate_at(4:5, ~ ifelse(is.numeric(.), NA, NA_character_)),
      start_row = 2
    ) %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(matriz_paisaje)),
      bold = TRUE
    ) %>%
    openxlsx2::wb_add_fill(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(matriz_paisaje)),
    ...
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(matriz_paisaje),
        rows = 1:(nrow(matriz_paisaje) + 2)
      ),
      inner_hgrid = "thin",
      inner_vgrid = "thin",
      inner_hcolor = openxlsx2::wb_color(hex = "#6F7B62"),
    ...
    openxlsx2::wb_add_formula(
      x = sprintf(
        "%s-%s",
        openxlsx2::wb_dims(x = matriz_paisaje, cols = 2, from_row = 2),
        openxlsx2::wb_dims(x = matriz_paisaje, cols = 3, from_row = 2)
      ),
      dims = openxlsx2::wb_dims(x = matriz_paisaje, cols = 4, from_row = 2),
      array = TRUE,
      cm = TRUE
    ) %>%
    openxlsx2::wb_add_formula(
      x = sprintf(
        "ROUND(LN(%s/%s)*100,2)",
        openxlsx2::wb_dims(x = matriz_paisaje, cols = 4, from_row = 2),
        openxlsx2::wb_dims(x = matriz_paisaje, cols = 2, from_row = 2)
      ),
      dims = openxlsx2::wb_dims(x = matriz_paisaje, cols = 5, from_row = 2),
      array = TRUE,
      cm = TRUE
    )
  for (i in c(2:3)) {
    wb <- wb %>%
      openxlsx2::wb_add_formula(
        x = sprintf(
          "SUM(%s)",
          openxlsx2::wb_dims(
            x = matriz_paisaje %>% dplyr::slice(1:(nrow(matriz_paisaje) - 3)),
            from_row = 2,
            cols = i
          )
        ),
      ...
      openxlsx2::wb_add_formula(
        x = sprintf(
          ...
          openxlsx2::wb_dims(
            ...
            from_row = 2,
            cols = i,
            rows = which(matriz_paisaje$Subuso == 'Otros usos sin vegetación')
          )
        ),
        dims = openxlsx2::wb_dims(
          x = matriz_paisaje,
          from_row = 2,
          cols = i,
          rows = which(matriz_paisaje$Subuso == 'Total')
        )
      )
  }
  wb <- wb %>%
    ...
    openxlsx2::wb_add_numfmt(
      dims = openxlsx2::wb_dims(cols = 2:5, rows = 1:nrow(matriz_paisaje) + 2),
      numfmt = "#,##0.00"
    ) %>%
  ...
  wb <- wb %>%
    ...
    openxlsx2::wb_add_data(
      x = "Tabla de evaluación de los 19 parámetros del análisis de amenazas para la especie en la cuenca",
      dims = "A1"
    ) %>%
    openxlsx2::wb_add_font(
      dims = "A1",
    ...
    openxlsx2::wb_add_cell_style(dims = "A1", horizontal = "center") %>%
    openxlsx2::wb_merge_cells(
      dims = openxlsx2::wb_dims(cols = 1:ncol(tabla_eval), rows = 1),
      solve = TRUE
    ) %>%
    openxlsx2::wb_add_data(
      x = tabla_eval %>%
        dplyr::mutate_at(5, ~ ifelse(is.numeric(.), NA, NA_character_)),
      start_col = 1,
      start_row = 2
    ) %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(tabla_eval)),
      bold = TRUE
    ) %>%
    openxlsx2::wb_add_fill(
      dims = openxlsx2::wb_dims(rows = 2, cols = 1:ncol(tabla_eval)),
    ...
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(
        cols = 1:ncol(tabla_eval),
        rows = 1:(nrow(tabla_eval) + 2)
      ),
      inner_hgrid = "thin",
      inner_vgrid = "thin",
      inner_hcolor = openxlsx2::wb_color(hex = "#6F7B62"),
    ...
    openxlsx2::wb_add_font(dims = "C22:E22", bold = TRUE) %>%
    ...
    openxlsx2::wb_merge_cells(dims = "C22:D22", solve = TRUE) %>%
    openxlsx2::wb_set_col_widths(cols = 1:5, widths = c(36, 85, 10, 12, 16))

  if (is.null(spp_acomp)) {
    wb <- wb %>%
      openxlsx2::wb_add_fill(
        sheet = "Tabla de Evaluación",
        dims = "C14:C15",
        color = openxlsx2::wb_color("yellow")
      )
  }
  if (is.null(prop)) {
    wb <- wb %>%
